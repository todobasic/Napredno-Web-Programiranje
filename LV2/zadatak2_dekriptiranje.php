...
$target_dir = "uploads/";
$allowed_types = array('pdf', 'jpeg', 'png', 'jpg');
...
foreach($files as $file) {
    ...
    if(strpos($file, "encrypted_") === 0) {
        ...
        $iv = substr($file_contents, 0, openssl_cipher_iv_length('AES-128-CTR'));
        $encrypted_data = substr($file_contents, openssl_cipher_iv_length('AES-128-CTR'));
        $key = md5('jed4n j4k0 v3l1k1 kljuc');
        $decrypted_data = openssl_decrypt($encrypted_data, 'AES-128-CTR', $key, OPENSSL_RAW_DATA, $iv);
